...
export default function AgregarPatrocinadorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.imageUrl instanceof File) {
        ...
        reader.onload = async () => {
          try {
            ...
            if (response.ok) {
              toast.success('Patrocinador creado exitosamente')
              router.push('/general/gestion/patrocinadores')
            } else {
              const errorData = await response.json()
              toast.error(errorData.error || 'Error al crear patrocinador')
            }
          } catch (error) {
            console.error('Error creating sponsor:', error)
            toast.error('Error al crear patrocinador')
          } finally {
      ...
      // If imageUrl is already a string (base64 or URL)
      const response = await fetch('/api/sponsors', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    <div className="p-6 pt-20 md:pt-6">
      ...
      <div className="mb-8">
        ...
        <p className="font-metropolis font-regular text-lg" style={{ color: '#4A739C' }}>
          Agrega un nuevo patrocinador a la plataforma
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="mb-6">
            ...
            <div className="mt-1">
              {formData.imageUrl ? (
                ...
                <div
                  className={`border-2 border-dashed rounded-lg p-6 text-center hover:border-[#5A6F80] transition-colors ${
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {isSubmitting ? 'Creando...' : 'Crear Patrocinador'}
          </button>
        </div>
      </form>
    </div>
