...
import { FAQFormData } from './useFAQForm'
...
export const useFAQTranslation = () => {
  ...
  const translateToEnglish = useCallback(async (
    ...
    setFormDataEnglish: React.Dispatch<React.SetStateAction<FAQFormData>>
  ) => {
    ...
    try {
      ...
      const textsToTranslate = [
        currentFormData.question_es || '',
        currentFormData.answer_es || ''
      ...
      if (translations && translations.length > 0) {
        ...
        const newEnglishData = {
          ...
          // Map question
          question_en: currentFormData.question_es && currentFormData.question_es.trim() ? translations[translationIndex++] : '',
          // Map answer
          answer_en: currentFormData.answer_es && currentFormData.answer_es.trim() ? translations[translationIndex++] : ''
        }
